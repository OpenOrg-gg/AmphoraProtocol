...
/*
AMP Locking contract for https://www.convexfinance.com/
AMP locked in this contract will be entitled to voting rights for the Convex Finance platform
...
contract CvxLockerV2 is ReentrancyGuard, Ownable {
    ...
    //token constants
    IERC20 public constant stakingToken = IERC20(0x4e3FBD56CD56c3e72c1403e103b45Db9da5B9D2B); //amph
    address public constant amphCrv = address(0x62B9c7356A2Dc64a1969e19C23e4f579F9810Aa7);
    ...
    address public constant amphcrvStaking = address(0x3Fe65692bfCD0e6CF84cB1E7d24108E434A7587e);
    address public vlAmph;
    ...
    constructor(address _vlAmph) public Ownable() {
    ...
    //Set the staking contract for the underlying amph
    ...
    //set approvals for staking amph and amphcrv
    function setApprovals() external {
        IERC20(amphCrv).safeApprove(amphcrvStaking, 0);
        IERC20(amphCrv).safeApprove(amphcrvStaking, uint256(-1));
    ...
    function _lock(address _account, uint256 _amount, uint256 _spendRatio, bool _isRelock) internal {
        ...
        //Add Users vlAMPH
        IvlAmph(vlAmph).mint(_account, _amount);
    ...
    function _processExpiredLocks(address _account, bool _relock, uint256 _spendRatio, address _withdrawTo, address _rewardAddress, uint256 _checkDelay) internal updateReward(_account) {
        ...
        uint256 amountToBurn = locked + boostedAmount;
        IvlAmph(vlAmph).burn(_account, amountToBurn);
        ...
        if (reward > 0) {
            ...
            //preallocate enough amph from stake contract to pay for both reward and withdraw
            allocateAMPForTransfer(uint256(locked));

            //reduce return amount by the kick reward
            locked = locked.sub(reward.to112());
            
            //transfer reward
            transferAMP(_rewardAddress, reward, false);

            emit KickReward(_rewardAddress, _account, reward);
        }else if(_spendRatio > 0){
            //preallocate enough amph to transfer the boost cost
            allocateAMPForTransfer( uint256(locked).mul(_spendRatio).div(denominator) );
        }
        ...
        if (_relock) {
            ...
            transferAMP(_withdrawTo, locked, true);
        }
    }
    ...
    //pull required amount of amph from staking for an upcoming transfer
    function allocateAMPForTransfer(uint256 _amount) internal{
    ...
    function transferAMP(address _account, uint256 _amount, bool _updateStake) internal {
        //allocate enough amph from staking for the transfer
        allocateAMPForTransfer(_amount);
        //transfer
    ...
    //calculate how much amph should be staked. update if needed
    ...
    function getReward(address _account, bool _stake) public nonReentrant updateReward(_account) {
        for (uint i; i < rewardTokens.length; i++) {
            ...
            if (reward > 0) {
                ...
                if (_rewardsToken == amphCrv && _stake) {
                    IRewardStaking(amphcrvStaking).stakeFor(_account, reward);
                } else {
