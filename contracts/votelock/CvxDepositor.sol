...
interface IStaker{
    ...
    function withdrawLocksTo(address _address) external;
...
contract CvxDepositor{
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;

    address public constant cvx = address(0x4e3FBD56CD56c3e72c1403e103b45Db9da5B9D2B);
    address public constant escrow = address(0x72a19342e8F1838460eBFCCEf09F6585e32db86E);
    uint256 private constant MAXTIME = 4 * 364 * 86400;
    uint256 private constant WEEK = 7 * 86400;

    uint256 public lockIncentive = 10; //incentive to users who spend gas to lock cvx
    ...
    //lock convex
    function _lockConvex() internal {
        uint256 cvxBalance = IERC20(cvx).balanceOf(address(this));
        if(cvxBalance > 0){
            IERC20(cvx).safeApprove(staker, 0);
            IERC20(cvx).safeApprove(staker, cvxBalance);
            IStaker(staker).lock(cvxBalance, 0);
        }
        //increase amount
    }

    function lockConvex() external {
        _lockConvex();
    ...
    function depositAll(bool _lock, address _stakeAddress) external{
        uint256 cvxBal = IERC20(cvx).balanceOf(msg.sender);
        deposit(cvxBal,_lock,_stakeAddress);
    }
}